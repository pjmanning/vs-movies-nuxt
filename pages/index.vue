...
const { pending: popularPending, data: popular } = useFetch('/api/movies?type=popular', {
  lazy: true,
})
const { pending: top_ratedPending, data: topRated } = useFetch('/api/movies?type=top_rated', {
  lazy: true,
})
const { pending: upcomingPending, data: upcoming } = useFetch('/api/movies?type=upcoming', {
  lazy: true,
})
const { pending: now_playingPending, data: nowPlaying } = useFetch('/api/movies?type=now_playing', {
  lazy: true,
})
  ...
  <div v-else>
    <div v-for="category in sections">
      <div class="mt-5 flex items-center px-10 py-3">
        ...
        <!-- <NuxtLink href="/movies/category/popular" class="" n-link="">Explore more</NuxtLink> -->
      </div>

      <div class="relative">
        <div class="overflow-y-auto">
          <ul class="flex w-max gap-2 p-2 px-10">
            <NuxtLink v-for="movie in category.movies.results" :key="movie.id" :to="`/movie/${movie.id}`" class="w-40 flex-1 pb-2 md:w-60">
