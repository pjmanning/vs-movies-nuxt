    ...
    <nav class="mx-auto flex max-w-7xl items-center justify-between p-6 lg:px-8" aria-label="Global">
      <div class="flex flex-1">
        <div class="hidden lg:flex lg:gap-x-12">
          <button v-for="item in navigation" :key="item.name" class="w-full text-left font-semibold leading-6 text-gray-900" @click="changePage(item)">{{ item.name }}</button>
        </div>
      ...
      <div class="flex flex-1 justify-end">
        <NuxtLink to="/search"><UIcon name="i-heroicons-magnifying-glass-20-solid" class="h-6 w-6 text-gray-700" aria-hidden="true" /></NuxtLink>
      </div>
    </nav>
